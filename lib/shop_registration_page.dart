...
class _ShopRegistrationPageState extends State<ShopRegistrationPage> {
  ...
  Future<String?> uploadImage(File imageFile, String folder) async {
    try {
      ...
      Reference ref =
          FirebaseStorage.instance.ref().child('$folder/$fileName.jpg');
  ...
  Future<void> _submitForm() async {
    ...
    if (_cnicFrontImage != null) {
      _cnicFrontUrl = await uploadImage(_cnicFrontImage!, "cnic_images");
    }
    if (_cnicBackImage != null) {
      _cnicBackUrl = await uploadImage(_cnicBackImage!, "cnic_images");
    }
    if (_shopImage != null) {
      _shopImageUrl = await uploadImage(_shopImage!, "shop_images");
    }
            ...
            onPressed: () async {
              if (_selectedLocation != null) {
                ...
                _shopLocationController.text =
                    '${p.first.street}, ${p.first.locality}';
              }
              Navigator.pop(context);
            },
  ...
  Widget field({
    ...
    return Padding(
      ...
      child: TextFormField(
        ...
        readOnly: readOnly,
        keyboardType: keyboard, // ✅ mapped from 'keyboard' param
    ...
    return Row(
      ...
      children: List.generate(3, (i) {
        return Container(
          ...
          child: Text("${i + 1}",
              style: TextStyle(
                  color: _currentStep == i ? white : darkBlue,
  ...
  Widget imageUploadBox(
      String title, IconData icon, File? imageFile, VoidCallback onTap) {
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            Expanded(
                child: Text(title, style: const TextStyle(color: darkBlue))),
            imageFile != null
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title:
            const Text("Shop Registration", style: TextStyle(color: white)),
      ),
      body: Form(
        key: _formKey,
        child: Column(
                    ...
                    child: Column(
                      children: [
                        ...
                        field(
                          ...
                          icon: Icons.person,
                          validator: (v) =>
                              v!.isEmpty ? "Owner name is required" : null,
                        ),
                        field(
                        ...
                        field(
                          ...
                          label: "Phone Number",
                          icon: Icons.phone,
                          keyboard: TextInputType.number, // ✅ FIXED
                          inputFormatters: phoneFormatter,
                          validator: (v) => RegExp(r'^03\d{2}-\d{7}$')
                                  .hasMatch(v!)
                              ? null
                              : "Phone must be 03xx-xxxxxxx",
                        ),
                        const SizedBox(height: 12),
                        imageUploadBox("Upload CNIC Front Image", Icons.badge,
                            _cnicFrontImage, () async {
                          File? img = await pickImage();
                          if (img != null) setState(() => _cnicFrontImage = img);
                        }),
                        const SizedBox(height: 12),
                        imageUploadBox("Upload CNIC Back Image", Icons.badge,
                            _cnicBackImage, () async {
                          File? img = await pickImage();
                          if (img != null) setState(() => _cnicBackImage = img);
                        }),
                        const Spacer(),
                    ...
                    child: Column(
                      children: [
                        ...
                        field(
                          ...
                          icon: Icons.store,
                          validator: (v) =>
                              v!.isEmpty ? "Shop name required" : null,
                        ),
                        DropdownButtonFormField(
                          value: _shopCategory,
                          decoration: InputDecoration(
                            prefixIcon: const Icon(Icons.category, color: orange),
                            floatingLabelStyle: const TextStyle(color: darkBlue),
                        ...
                        field(
                          ...
                          validator: (v) => RegExp(r'^\d{5}-\d{7}-\d{1}$')
                                  .hasMatch(v!)
                              ? null
                        ...
                        const SizedBox(height: 12),
                        imageUploadBox(
                            "Upload Shop Image", Icons.store, _shopImage, () async {
                          File? img = await pickImage();
                          if (img != null) setState(() => _shopImage = img);
                        }),
                        const SizedBox(height: 14),
                        field(
                    ...
                    child: Column(
                      children: [
                        ...
                        field(
                          ...
                          readOnly: true,
                          validator: (v) =>
                              v!.isEmpty ? "Please select location" : null,
                        ),
